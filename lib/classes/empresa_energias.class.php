...
class empresa_energias extends entity {
	...
	public function update($updates, $table = null) {
		...
		$sql = $this->buildUpdateQueryWhere($updates, array('empresa' => $_SESSION['u']['enterprise'], 'energia' => $updates['energia']), $table);
		//var_dump($sql); die;
		$this->sql($sql);
	}
	
	public function getRecordE($id){
		$req = "SELECT * FROM $this->table WHERE energia = '$id' AND empresa = '".$_SESSION['u']['enterprise']."' LIMIT 1";
		$res = $this->select($req);
		return $res[0];
	}
