...
class empresa_residuos_tratamiento extends entity {
	...
	public function update($updates, $table = null) {
		...
		$sql = $this->buildUpdateQueryWhere($updates, array('empresa' => $_SESSION['u']['enterprise'], 'tratamiento_residuo' => $updates['tratamiento_residuo'], 'residuo' => $updates['residuo']), $table);
		//var_dump($sql); die;
		$this->sql($sql);
	}
	
	public function getRecordECrop($id, $id2){
		$req = "SELECT * FROM $this->table WHERE residuo = '$id' AND tratamiento_residuo = '$id2' AND empresa = '".$_SESSION['u']['enterprise']."' LIMIT 1";
		$res = $this->select($req);
		return $res[0];
	}
	
	public function delProd($id, $id2) {
		$req = "SET FOREIGN_KEY_CHECKS=0; DELETE FROM $this->table WHERE empresa = '".$_SESSION['u']['enterprise']."' AND residuo = '".$id."' AND es_produccion = '1' AND tratamiento_residuo = '".$id2."';";
		//var_dump($req); die;
		$this->sql($req);
	}
