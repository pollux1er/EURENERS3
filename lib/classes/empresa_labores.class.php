...
class empresa_labores extends entity {
	...
	public function updateE($data) {
		...
		foreach($data as $k => $v) {
			if($k != 'labor' || $k != 'empresa')
				$updates .= ($v == '') ? "$k = NULL, " : "$k = '$v', ";
		}
		$updates = removeLastChar($updates);
		$query .= "$updates WHERE empresa = '".$_SESSION['u']['enterprise']."' AND labor = '".$data['labor']."'";
		//var_dump($query); die;
		$this->update($query);
	}
	
	public function getRecordE($id){
		$req = "SELECT * FROM $this->table WHERE labor = '$id' AND empresa = '".$_SESSION['u']['enterprise']."' LIMIT 1";
		$res = $this->select($req);
		return $res[0];
	}
	
	public function del($id) {
		$req = "SET FOREIGN_KEY_CHECKS=0; DELETE FROM $this->table WHERE empresa = '".$_SESSION['u']['enterprise']."' AND labor = '".$id."';";
		//var_dump($req); die;
		$this->sql($req);
	}
