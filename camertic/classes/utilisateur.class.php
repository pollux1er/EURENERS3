...
class utilisateur extends bd {
	...
	*/
	...
	public function simpleLogin($username, $pass) {
		...
		$requete = "SELECT * FROM $this->tableusers WHERE usuario = '$this->utilisateur' AND active NOT IN(0) LIMIT 1;";
		...
		if(parent::countResult($requete) == 0) {
			...
			if($userdatas->clave == $hash) {
				$this->isLoggedIn = true;
				$this->authenticateUser($userdatas->usuario);
				return true;
			}
			else {
				return false;
	...
	public function createUserSimple($post) {
		...
		// insertion des parametres utilisateur
		$req = "INSERT INTO $this->tableusers(usuario, nombre, grupo, clave, apellidos, email, telefono, idioma, empresa, active) VALUES ('$login', '$nom', '$groupe', '$hashedPassword', '$apellidos', '$email', '$tel', '$idioma', $enterprise, '1')";
		//var_dump($req); die();
		$res = $this->insert($req);
	...
	public function editUserSimple($post) {
		...
		// insertion des parametres utilisateur
		if($hashedPassword == '')
			$req = "UPDATE $this->tableusers SET nombre = '$nom', grupo = '$groupe', apellidos = '$apellidos', email = '$email', usuario = '$login', telefono = '$tel', idioma = '$idioma' $fields WHERE usuario = '".$post['olduser']."'";
		else
			$req = "UPDATE $this->tableusers SET clave = '$hashedPassword',nombre = '$nom', grupo = '$groupe', apellidos = '$apellidos', email = '$email', telefono = '$tel', idioma = '$idioma', usuario = '$login' $fields WHERE usuario = '".$post['olduser']."'";
		//var_dump($req); die;
		$res = $this->insert($req);
	...
	public function blockUser($post) {
		$id = $post['user'];
		$req = "UPDATE $this->tableusers SET active = '-1' WHERE usuario = '$id';";
		return $this->update($req);
	}
	
	public function unblockUser($post) {
		$id = $post['user'];
		$req = "UPDATE $this->tableusers SET active = '1' WHERE usuario = '$id';";
		return $this->update($req);
	}
	...
	public function deleteUser($post) {
		$id = $post['user'];
		$req = "DELETE FROM $this->tableusers WHERE usuario = '$id';";
		return $this->update($req);
	}
	...
	public function getUser($iduser) {
		$requete = "SELECT * FROM $this->tableusers WHERE usuario = '$iduser' LIMIT 1;";
	...
	public function getListUsers() {
		$req = "SELECT u.usuario, u.nombre AS name, u.active, u.telefono AS tel, u.empresa AS enterprise, u.email, ug.nombre AS grupo FROM $this->tableusers AS u ";
	...
	public function getListUsersFromEnterprise($id) {
		...
		$req .= " WHERE u.active NOT IN(-1) AND ue.identificador = " . $id;
		//var_dump($req); die;
		return $this->select($req);
	}
	...
	public function getUserDatas($ids){
		$requete = "SELECT * FROM $this->tableusers WHERE usuario = '$ids' LIMIT 1;";
	...
	*/
	...
	//		$_SESSION['u']['last_login'] = $this->userDatas->last_login;
			...
			$_SESSION['u']['idgroupe'] = $this->userDatas->grupo;
			$_SESSION['u']['lang'] = $this->userDatas->idioma;
			$_SESSION['u']['utilisateur'] = $this->userDatas->usuario;
			$_SESSION['u']['enterprise'] = $this->userDatas->empresa;
			$_SESSION['iduser']= $this->userDatas->identificador;
		}
		else {
