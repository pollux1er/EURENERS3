...
function addMatP() {
	...
	var materia_prima = jQuery('#materia_prima').val();
	if(materia_prima == '') {
		alert('You must fill the producto final!');
		return false;
	}
...
function deleteMatP(idc) {
	if (confirm("You want to delete the Materia Prima?")) { 
		jQuery.ajax({
		  ...
		  success: function(html){
			if(html==""){
				jQuery('#response').html('<span></span>Materia Prima succesfully deleted.');
				jQuery('#message').fadeIn("slow");
				jQuery('#'+idc).fadeOut("slow");
				setTimeout(function() {
					$('#'+idc).remove();
